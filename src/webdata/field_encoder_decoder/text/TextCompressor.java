...
public class TextCompressor {
	...
	private final UnaryCoder reviewLengthCoder;
	private final UnaryCoder tokenFrequencyCoder;
	private final UnaryCoder tokenCollectionFrequencyCoder;
	...
	TextCompressor() {
		this.reviewLengthCoder = new DeltaCode();
		this.tokenFrequencyCoder = new GammaCode();
		this.tokenCollectionFrequencyCoder = new GammaCode();

		this.dictCoder = new KInKMinusOneFrontCoding();
		this.termPtrCoder = new DeltaCodeGap(1);
		this.lengthsCoder = new GammaCode();
		this.prefixesCoder = new GammaCode(1); // can be  0...
		this.invertedIndexCoder = new DeltaCode();
		this.invertedIndexPtrCoder = new DeltaCodeGap(1);
	}


	public List<Byte> compressReviewLength(List<Integer> lengths) {
		return this.reviewLengthCoder.encode(lengths);
	}

	public List<Integer> decompressReviewLength(byte[] bytes, int size) {
		return this.reviewLengthCoder.decode(size, bytes);
	}
	...
	public List<Byte> compressTokenCollectionFrequency(List<Integer> integerList) {
		return this.tokenCollectionFrequencyCoder.encode(integerList);
	}

	public List<Integer> decompressCollectionFrequency(byte[] bytes, int size) {
		return this.tokenCollectionFrequencyCoder.decode(size, bytes);
	}
	...
	public InvertedCompressResult compressInvertedIndex(
		...
		List<List<Integer>> invertedRIds = TextUtils.invertedRIds(tokenFrequency, invertedIndexRIds, invertedIndexFreqInRId);
	...
	public List<Integer> decompressInvertedIndex(byte[] data, int size) {
		List<Integer> decode = this.invertedIndexCoder.decode(size * 2, data);
		Utils.addLastValueForEachM(decode, 2);
		return decode;
	}

	public int mergeInputsToOutput(TextInputStreamWrapper wrapper1, TextInputStreamWrapper wrapper2, TextOutputStreamWrapper outputStreamWrapper) {
	...
	public int megaMerge(
			List<TextInputStreamWrapper> wrappers,
			TextOutputStreamWrapper outputStreamWrapper
	) {
		int size = wrappers.size();
		List<WordInfoIter> wordInfoIters = new ArrayList<>(size);
		for (TextInputStreamWrapper inputStreamWrapper : wrappers) {
	...
	private void addInfo(MergerKInKMinusOneFrontCoding dictMerger,
		...
		int lastValue = 0;
		for (int i = 0; i < invertedIndex.size(); i += 2) {
		...
		writer.writeInfoToFile(info, freq, collectionFreq);
	}
	...
	public void mergeFileLength(TextInputStreamWrapper wrapper1, int amountOfReviews1, TextInputStreamWrapper wrapper2, int amountOfReviews2, TextOutputStreamWrapper outputStreamWrapper) {
		this.reviewLengthCoder.mergeTwoInputToOutput(amountOfReviews1, amountOfReviews2,
				wrapper1.reviewLenInputStream, wrapper2.reviewLenInputStream,
				outputStreamWrapper.reviewLenOutputStream);
	}

	public void megaMergeFileLength(List<TextInputStreamWrapper> l, List<Integer> amountOfReviews, TextOutputStreamWrapper outputStreamWrapper) {
		this.reviewLengthCoder.mergeInputsToOutput(
				amountOfReviews, l.stream().map((w) -> w.reviewLenInputStream).collect(Collectors.toList()),
				outputStreamWrapper.reviewLenOutputStream
		);
	}


	public static class TextInputStreamWrapper {
		...
		private final InputStream collectionFreqInputStream;
		private final InputStream freqInputStream;
		private final InputStream reviewLenInputStream;
		private final InputStream invertedIndexInputStream;
		private final InputStream invertedIndexPtrInputStream;

		TextInputStreamWrapper(String dir) throws FileNotFoundException {
			String f1 = Paths.get(dir, TextConstants.TERM_FILE_NAME.value).toString();
			String f2 = Paths.get(dir, TextConstants.PREFIX_FILE_NAME.value).toString();
			String f3 = Paths.get(dir, TextConstants.WORD_LEN_FILE_NAME.value).toString();
			String f4 = Paths.get(dir, TextConstants.TERM_PTR_FILE_NAME.value).toString();
			String f5 = Paths.get(dir, TextConstants.COLLECTION_FREQ_FILE_NAME.value).toString();
			String f6 = Paths.get(dir, TextConstants.FREQ_FILE_NAME.value).toString();
			String f7 = Paths.get(dir, TextConstants.REVIEW_LEN_FILE_NAME.value).toString();
			String f8 = Paths.get(dir, TextConstants.INVERTED_INDEX_FILE_NAME.value).toString();
			String f9 = Paths.get(dir, TextConstants.INVERTED_INDEX_PTR_FILE_NAME.value).toString();
			...
			collectionFreqInputStream = new FileInputStream(f5);
			freqInputStream = new FileInputStream(f6);
			reviewLenInputStream = new FileInputStream(f7);
		...
		void close() throws IOException {
			...
			collectionFreqInputStream.close();
			freqInputStream.close();
			reviewLenInputStream.close();
			invertedIndexInputStream.close();
			invertedIndexPtrInputStream.close();
		}
	}

	public static class TextOutputStreamWrapper {
		...
		private final OutputStream collectionFreqOutputStream;
		private final OutputStream freqOutputStream;
		private final OutputStream reviewLenOutputStream;
		private final OutputStream invertedIndexOutputStream;
		private final OutputStream invertedIndexPtrOutputStream;

		TextOutputStreamWrapper(String dir) throws FileNotFoundException {
			String f1 = Paths.get(dir, TextConstants.TERM_FILE_NAME.value).toString();
			String f2 = Paths.get(dir, TextConstants.PREFIX_FILE_NAME.value).toString();
			String f3 = Paths.get(dir, TextConstants.WORD_LEN_FILE_NAME.value).toString();
			String f4 = Paths.get(dir, TextConstants.TERM_PTR_FILE_NAME.value).toString();
			...
			collectionFreqOutputStream = new FileOutputStream(f5);
			freqOutputStream = new FileOutputStream(f6);
			reviewLenOutputStream = new FileOutputStream(f7);
		...
		void close() throws IOException {
			...
			collectionFreqOutputStream.close();
			freqOutputStream.close();
			reviewLenOutputStream.close();
	...
	private class InfoWriter {
		...
		private final GammaCode.BitWriterToFile collectionFreqOutputStream;
		...
		InfoWriter(TextOutputStreamWrapper outputStreamWrapper) {
			termOutput = new BufferedOutputStream(outputStreamWrapper.termOutputStream);

			invertedIndexOutputStream = new GammaCode.BitWriterToFile(outputStreamWrapper.invertedIndexOutputStream);
			...
			collectionFreqOutputStream = new GammaCode.BitWriterToFile(outputStreamWrapper.collectionFreqOutputStream);
		...
		void writeFinalResult() {
			...
			collectionFreqOutputStream.writeFinalResult();
		...
		void writeInvertedIndexes(List<List<Integer>> inverteds) {
			int lastInvertedValue = 0;
			for (List<Integer> inverted : inverteds) {
				for (int i = 0; i < inverted.size(); i += 2) {
					...
					value = inverted.get(i + 1);
					invertedIndexCoder.encodeSingleNumber(this.invertedIndexOutputStream, value, 0);
				}
			}
		...
		void writeInfoToFile(MergerKInKMinusOneFrontCoding.WriteInfo info, int freq, int collectionFreq) {
			try {
			...
			tokenCollectionFrequencyCoder.encodeSingleNumber(this.collectionFreqOutputStream, collectionFreq, 0);
	...
	public class WordInfoIter implements Iterator<WordInfo> {
		...
		private final GammaCode.BitReaderFromFile collectionFreqReader;
		...
		public WordInfoIter(TextCompressor.TextInputStreamWrapper textInputStreamWrapper) {
			this.termInputStream = new BufferedInputStream(textInputStreamWrapper.termInputStream);
			this.invertedIndexInputStream =
					new BufferedInputStream(textInputStreamWrapper.invertedIndexInputStream);
			prefixReader = new GammaCode.BitReaderFromFile(textInputStreamWrapper.prefixInputStream);
			lengthReader = new GammaCode.BitReaderFromFile(textInputStreamWrapper.lengthInputStream);
			termPtrReader = new GammaCode.BitReaderFromFile(textInputStreamWrapper.termPtrInputStream);
			freqReader = new GammaCode.BitReaderFromFile(textInputStreamWrapper.freqInputStream);
			collectionFreqReader = new GammaCode.BitReaderFromFile(textInputStreamWrapper.collectionFreqInputStream);

			invertedIndexPtrReader =
					new GammaCode.BitReaderFromFile(textInputStreamWrapper.invertedIndexPtrInputStream);
		...
		private WordInfo readNextWord() {
			...
			int collectionFreq = tokenCollectionFrequencyCoder.getValue(this.collectionFreqReader, 0);
			...
			return new WordInfo(word, freq, collectionFreq, invertedIndex);
		}
